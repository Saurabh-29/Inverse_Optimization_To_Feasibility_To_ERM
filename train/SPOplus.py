...
class SPOplus():
    def __init__(self, config):
        ...
        lp_params = {"A": self.A, "b": self.b, "problem": "LP", "max_cap_on_pred": config["method"]["max_cap_on_pred"]}
    ...
    def criterion_with_c_pred(self, c_pred, c_gt,  x_target):
        x_pred = self.solver(c_pred, c_gt , self.lp_solver)
        return (0.5*((x_target - x_pred)**2).sum())/ x_target.shape[0]
        # return (0.5*((x_pred)**2).sum())/ x_target.shape[0]
...
class ShortestPath(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, c_pred, c_gt, solver):
        ctx.solver = solver
        ctx.weights = 2*c_pred.detach().cpu().numpy() - c_gt
        ctx.weights = np.maximum(ctx.weights, 0.0)
        # ctx.weights = np.clip(ctx.weights, 0, None)
